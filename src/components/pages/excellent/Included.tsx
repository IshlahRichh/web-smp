const Included = () => {
   return (
      <div className="tg-tour-about-inner mb-40">
         <div className="row">
            <div className="col-lg-7 col-11 m-auto m-lg-0">
               <h4 className="tg-tour-about-title mb-20">Kegiatan Program Kelas Excellent</h4>
               <div className="tg-tour-about-list  tg-tour-about-list-2">
                  <ul>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Upgrading Skill Class (Excellent Eduscience and Enterprenur)</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Excellent Competition</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Excellent Visit</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Excellent Expo</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Excellent Scientific Project</span>
                     </li>
                  </ul>
               </div>
            </div>
            {/* <div className="col-lg-7">
               <div className="tg-tour-about-list tg-tour-about-list-2">
                  <ul>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Pick and Drop Service</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">1 Meal Per Day</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Cruise Dinner & Music Event</span>
                     </li>
                     <li>
                        <span className="icon mr-10"><i className="fa-sharp fa-solid fa-check fa-fw"></i></span>
                        <span className="text">Visit 7 Best Places</span>
                     </li>
                  </ul>
               </div>
            </div> */}
         </div>
      </div>
   )
}

export default Included
